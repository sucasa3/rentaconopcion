...
function AgentsLandingPage() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-background text-foreground"><SiteHeader /><main>
    <section className="border-b border-border bg-surface-warm">
      <div className="mx-auto grid max-w-6xl gap-8 px-5 pb-10 pt-9 md:grid-cols-[minmax(0,1fr)_minmax(360px,0.82fr)] md:items-center md:pb-14 md:pt-14">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-status-opportunity">{t("pub.agents.eyebrow")}</p>
          <h1 className="mt-3 max-w-3xl text-[2.5rem] font-semibold leading-[1.04] sm:text-5xl lg:text-6xl">{t("pub.agents.h1")}</h1>
          <p className="mt-5 max-w-xl text-base leading-relaxed text-muted-foreground sm:text-lg">{t("pub.agents.sub")}</p>
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <Button asChild size="lg" className="min-h-12 bg-sucasa-orange text-sucasa-orange-foreground hover:bg-sucasa-orange/90"><Link to="/agent-start" search={{ source: "agents_hero" }} onClick={() => track("agent_start_clicked")}>{t("pub.agents.cta_primary")} <ArrowRight /></Link></Button>
            <Button asChild size="lg" variant="outline" className="min-h-12"><Link to="/agents/pricing" onClick={() => track("agent_pricing_clicked")}>{t("pub.agents.cta_pricing")}</Link></Button>
          </div>
    ...
    <section className="bg-sucasa-navy py-12 text-primary-foreground sm:py-16"><div className="mx-auto grid max-w-6xl gap-6 px-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
      <div><p className="text-sm font-semibold text-sucasa-orange">{t("pub.agents.final_eyebrow")}</p><h2 className="mt-2 text-3xl font-semibold sm:text-4xl">{t("pub.agents.final_title")}</h2><p className="mt-3 max-w-2xl text-primary-foreground/70">{t("pub.agents.final_sub")}</p></div>
      <div className="flex flex-col gap-3 sm:flex-row"><Button asChild size="lg" className="min-h-12 bg-sucasa-orange text-sucasa-orange-foreground hover:bg-sucasa-orange/90"><Link to="/agents/pricing" onClick={() => track("agent_pricing_clicked")}>{t("pub.agents.final_cta")} <ArrowRight /></Link></Button><Button asChild size="lg" variant="outline" className="min-h-12 border-primary-foreground/25 bg-transparent text-primary-foreground hover:bg-primary-foreground/10"><Link to="/agent-start" search={{ source: "agents_final" }} onClick={() => track("agent_start_clicked")}>{t("pub.agents.final_free")}</Link></Button></div>
    </div></section>
  </main><SiteFooter /></div>;
}
