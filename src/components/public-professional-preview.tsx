  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card shadow-elevated">
      ...
      <div className="grid gap-3 p-3 sm:p-4">
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-lg bg-surface-warm p-3">
        ...
        <article className="overflow-hidden rounded-lg border border-border bg-card">
          ...
          <div className="p-3 sm:p-4">
            ...
            <div className="mt-3 border-l-2 border-sucasa-orange pl-3"><p className="text-[10px] font-semibold uppercase text-muted-foreground">{t("pub.home.demo.why")}</p><p className="mt-1 text-sm leading-relaxed">{t("pub.home.demo.agent_why")}</p></div>
            <div className="mt-3 rounded-lg border border-surface-intelligence-border bg-surface-intelligence p-3">
              ...
              <p className="mt-1.5 text-xs leading-relaxed text-foreground">{t("pub.home.demo.agent_say")}</p>
            </div>
            <div className="mt-3 flex items-center justify-between gap-3"><p className="flex min-w-0 items-center gap-2 text-xs font-semibold text-sucasa-navy"><MessageSquareText className="h-4 w-4 shrink-0 text-intelligence-accent" />{t("pub.home.demo.agent_next")}</p><ArrowUpRight className="h-4 w-4 shrink-0 text-primary" /></div>
          </div>
        </article>
      </div>
      <DemoFooter />
...
export function PublicLenderTodayPreview() {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card shadow-elevated">
      ...
      <div className="grid gap-3 p-3 sm:p-4">
        <div className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-3 rounded-lg border border-surface-intelligence-border bg-surface-intelligence p-3">
        ...
        <article className="rounded-lg border border-border bg-card p-3 sm:p-4">
          ...
          <div className="mt-3 border-l-2 border-sucasa-orange pl-3"><p className="text-[10px] font-semibold uppercase text-muted-foreground">{t("pub.home.demo.why")}</p><p className="mt-1 text-sm leading-relaxed">{t("pub.home.demo.lender_why")}</p></div>
          <div className="mt-3 flex items-center justify-between gap-3 rounded-lg bg-surface-warm p-3"><p className="flex min-w-0 items-center gap-2 text-xs font-semibold text-sucasa-navy"><CheckCircle2 className="h-4 w-4 shrink-0 text-status-positive" />{t("pub.home.demo.lender_next")}</p><ArrowUpRight className="h-4 w-4 shrink-0 text-primary" /></div>
        </article>
      </div>
      <DemoFooter />
    </div>
...
function PreviewHeader({ title }: { title: string }) {
  const { t } = useLanguage();
  return <div className="flex items-center justify-between gap-3 border-b border-border bg-sucasa-navy px-4 py-3 text-primary-foreground"><p className="font-semibold">{title}</p><span className="rounded-full bg-primary-foreground/10 px-2.5 py-1 text-[9px] font-semibold">{t("pub.home.demo_badge")}</span></div>;
}

function ContextTile({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="min-w-0 rounded-lg bg-secondary p-2.5"><span className="text-intelligence-accent [&_svg]:h-3.5 [&_svg]:w-3.5">{icon}</span><p className="mt-2 text-[9px] font-semibold uppercase text-muted-foreground">{label}</p><p className="mt-1 text-[10px] font-semibold leading-snug text-sucasa-navy">{value}</p></div>;
}

function DemoFooter() {
  const { t } = useLanguage();
  return <p className="border-t border-border px-4 py-2.5 text-center text-[9px] text-muted-foreground">{t("pub.home.demo_all")}</p>;
}