...
import { Sparkles, Phone, Mail, Loader2, Check, ArrowRight } from "lucide-react";
...
type Signal = "strong" | "moderate";

export function RefiChip({
  signal,
  estSavingsMonthly,
  equityDollars,
  currentRate,
  ...
  const chipLabel =
    signal === "strong" && estSavingsMonthly && estSavingsMonthly > 50
      ? `Refi ready · save ~$${Math.round(estSavingsMonthly).toLocaleString()}/mo`
      : signal === "strong"
        ? "Refi ready · explore options"
        : "Explore refi options";

  const chipTone =
    signal === "strong"
      ? "bg-emerald-400/95 text-emerald-950 ring-emerald-200/60"
      : "bg-white/90 text-neutral-900 ring-white/60";

  return (
    <>
      <button
        onClick={() => setOpen(true)}
        className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-semibold shadow-lg ring-1 backdrop-blur transition hover:scale-[1.02] ${chipTone}`}
      >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Your matched lender</DialogTitle>
            <DialogDescription>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                ...
                <Stat
                  label="Est. savings"
                  value={
                    estSavingsMonthly && estSavingsMonthly > 0
                      ? `$${Math.round(estSavingsMonthly).toLocaleString()}/mo`
                      : "—"
                  }
                />
              </div>
