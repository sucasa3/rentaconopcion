...
export function CampaignsWorkspace({ orgs }: { orgs: Array<{ id: string; name: string }> }) {
  ...
  const toggle = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("biz.campaigns.updated"));
    ...
    <div className="space-y-6">
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <Eye className="h-3 w-3" /> {t("biz.campaigns.preview")}
                </button>
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-full border border-border px-3 py-2 text-xs font-semibold hover:bg-muted disabled:opacity-50"
                >
                  <Pencil className="h-3 w-3" /> {t("biz.campaigns.edit")}
                </button>
                <button
                ...
                >
                  {on ? t("biz.campaigns.on") : t("biz.campaigns.off")}
                </button>
              </div>
            </div>
          );
      ...
      {!!state?.sends?.length && (
        <div className="rounded-3xl border border-border bg-card p-6 shadow-soft">
          <h2 className="text-base font-semibold">{t("biz.campaigns.recent_messages")}</h2>
