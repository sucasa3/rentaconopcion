  ...
  .handler(async ({ data, context }) => {
    ...
    return {
      ...
      brief: {
        ...
        metrics: {
          estimatedValueCents: person.estimatedValueCents,
          estimatedEquityCents: person.estimatedEquityCents,
          estimatedLtvPct: person.estimatedLtvPct,
          estimatedBalanceCents: person.estimatedBalanceCents,
          loanAgeYears: person.loanAgeYears,
          tenureYears: person.tenureYears,
          annualReviewDue: person.annualReviewDue,
          dataGap: person.dataGap,
          valueSource: person.valueSource,
        },
        reviews: person.reviews.slice(0, 3).map((review) => ({
          id: review.id,
          label: review.label,
          blurb: review.blurb,
          action: review.action,
          why: review.why.slice(0, 3),
        })),
        questions: [
