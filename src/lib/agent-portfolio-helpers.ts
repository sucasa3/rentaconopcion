...
export function extractOwnership(raw: unknown): OwnershipSummary {
  ...
  for (const key of ["owner1", "owner2", "owner3", "owner4"]) {
    const o = owner?.[key];
    if (!o) continue;
    const name = typeof o === "string" ? o : [o.firstnameandmi ?? o.firstname, o.lastname].filter(Boolean).join(" ");
  ...
  return {
    ...
    mailingDiffersFromSite: mailing && site ? mailing.trim().toLowerCase() !== site.trim().toLowerCase() : null,
  };
}
...
export function extractTaxTrend(raw: unknown, marketValue: number | null): TaxTrend {
  ...
  const history: any[] = Array.isArray((raw as any)?.property)
    ? (raw as any).property.map((row: any) => row?.assessment?.tax).filter(Boolean)
    : [];
  ...
  return {
    ...
    taxChangePct: latestTax && prevTax ? Math.round(((latestTax - prevTax) / prevTax) * 1000) / 10 : null,
    assessedToMarketPct: assessed && marketValue ? Math.round((assessed / marketValue) * 1000) / 10 : null,
  };
}
...
export function computeMoveScore(input: MoveScoreInput): MoveScore {
  ...
  if (listing && listing.listed_with_other_agent && (listing.status === "active" || listing.status === "pending")) {
    signals.push({ kind: "listed_elsewhere", label: "Listed with another agent", detail: listing.listing_agent_name ? `Represented by ${listing.listing_agent_name}. Quiet mode — no solicitation.` : "Quiet mode — value-only touches, no solicitation.", weight: 0, tone: "hold" });
    return { score: 0, band: "hold", signals, headline: "Quiet mode — currently represented" };
  }
  if (listing && (listing.status === "expired" || listing.status === "withdrawn")) {
    const w = 40;
    score += w;
    signals.push({ kind: "expired_listing", label: listing.status === "expired" ? "Listing expired" : "Listing withdrawn", detail: `Tried to sell${listing.list_price_cents ? ` at $${Math.round(listing.list_price_cents / 100).toLocaleString()}` : ""} and came off market. Highest-intent conversation you can have today.`, weight: w, tone: "hot" });
  }
  ...
  if (permitYears != null && permitYears <= 2 && (input.permitTotalValue ?? 0) >= 15000) {
    const w = 12;
    score += w;
    signals.push({ kind: "renovation", label: "Recent major permit", detail: `$${Math.round(input.permitTotalValue ?? 0).toLocaleString()} of permitted work in the last ${Math.max(1, Math.round(permitYears))} yr — value story for a CMA, and a reason to call.`, weight: w, tone: "warm" });
  }
  if ((input.taxChangePct ?? 0) >= 8) {
    const w = 8;
    score += w;
    signals.push({ kind: "tax_pressure", label: `Taxes up ${input.taxChangePct}%`, detail: "Assessment jump — a common trigger for downsizing or appeal conversations.", weight: w, tone: "info" });
  }
  if (input.ownerOccupied === false) {
    const w = 10;
    score += w;
    signals.push({ kind: "absentee", label: "Absentee owner", detail: "Mailing address differs from the property — likely a rental or second home.", weight: w, tone: "warm" });
  }
  ...
  return { score, band, signals, headline: top ? top.label : "No movement signals yet" };
}
...
export function computeListingReadiness(input: {
  ...
  const netProceeds = value != null ? Math.round(value * (1 - input.sellCostPct / 100) - (input.loanBalance ?? 0)) : null;
  const permitYears = yearsSince(input.lastPermitDate);
  const age = input.yearBuilt ? new Date().getFullYear() - input.yearBuilt : null;
  const checks: ReadinessCheck[] = [
    { key: "equity_covers_costs", label: "Equity clears selling costs", ok: netProceeds != null && netProceeds > 0, detail: netProceeds == null ? "Need a valuation to model net proceeds." : netProceeds > 0 ? `Est. $${netProceeds.toLocaleString()} at closing after ${input.sellCostPct}% costs.` : "Underwater after selling costs — not a listing conversation yet." },
    { key: "records", label: "Property records on file", ok: input.hasIntel, detail: input.hasIntel ? "Valuation, tax, sale and permit history cached." : "Pull property records to score this household." },
    { key: "condition", label: "Condition story", ok: (permitYears != null && permitYears <= 7) || (age != null && age <= 15), detail: permitYears != null && permitYears <= 7 ? "Recent permitted work supports a premium list price." : age != null && age <= 15 ? "Newer construction — limited prep expected." : "No recent permits — expect a pre-list prep conversation." },
    { key: "tenure_basis", label: "Past the 2-year basis window", ok: (input.tenureYears ?? 0) >= 2, detail: (input.tenureYears ?? 0) >= 2 ? "Qualifies for the capital-gains exclusion window." : "Under two years of ownership — tax hit on gains." },
    { key: "clear_of_listing", label: "Not represented elsewhere", ok: !(input.listing?.listed_with_other_agent && (input.listing.status === "active" || input.listing.status === "pending")), detail: input.listing?.listed_with_other_agent ? "Under another agent's agreement — value-only contact." : "No active representation on record." },
    { key: "contactable", label: "Reachable", ok: input.hasContact, detail: input.hasContact ? "Phone or email on file." : "No contact info — needs skip trace." },
  ];
  const score = Math.round((checks.filter((c) => c.ok).length / checks.length) * 100);
  const label: ListingReadiness["label"] = score >= 84 ? "list-ready" : score >= 50 ? "prep-needed" : "not-ready";
